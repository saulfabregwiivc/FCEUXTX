...
/*	{(uint8_t*)"",					135, Mapper135_Init}, */
	...
	{(uint8_t*)"S74LS374N",			150, BIC62_Init},
...
if (head.reserve[5] & 2)vt03_mode = 1;
	else vt03_mode = 0;						//mod: 4bpp tiles
	
if (head.reserve[5] & 4)vt03_mmc3_flag = 1; //mod

if (head.reserve[5] & 8)newopcodes = 1; //mod: LDZ TZA etc

	if(head.reserve[3]&0xC0)
		{
			switch(head.reserve[3]&0xC0)
			{
			case 0x40:
			wss = 352;
			break;
...
/* end */
	...
	VROM_size = head.VROM_size |((head.reserve[3]&0x0F)<<8);  //mod: more chr banks allowed
	...
	*/
	...
	if (strstr(name, "128_spr") || strstr(name, "256_spr"))sprites256 = 1; //mod: old hacks compatibility
