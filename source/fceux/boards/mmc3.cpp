...
void FixMMC3CHR(int V) {
	...
	{
	cwrap((cbase ^ 0x000), DRegBuf[0] & (~1));
	cwrap((cbase ^ 0x800), DRegBuf[0] | 1);
	cwrap((cbase ^ 0x1000), DRegBuf[1] & (~1));
	cwrap((cbase ^ 0x1800), DRegBuf[1] | 1);

	cwrap(cbase ^ 0x8000, DRegBuf[2]);
	cwrap(cbase ^ 0x8800, DRegBuf[3]);
	cwrap(cbase ^ 0x9000, DRegBuf[4]);
	cwrap(cbase ^ 0x9800, DRegBuf[5]);
	}
...
/*	FCEU_printf("bs %04x %02x\n",A,V); */
	...
	case 0x8001:
	{
		...
		case 1:
			if(vt03_mode)
			{
			cwrap((cbase ^ 0x1000), V & (~1));
			cwrap((cbase ^ 0x1800), V | 1);
			}
			else
			{
			cwrap((cbase ^ 0x800), V & (~1));
		...
		case 2:
			if(vt03_mode)
			cwrap(cbase ^ 0x8000, V);
			else cwrap(cbase ^ 0x1000, V);
			break;
		case 3:
			if(vt03_mode)
			cwrap(cbase ^ 0x8800, V);
			else cwrap(cbase ^ 0x1400, V);
			break;
		case 4:
			if(vt03_mode)
			cwrap(cbase ^ 0x9000, V);
			else cwrap(cbase ^ 0x1800, V);
			break;
		case 5:
			if(vt03_mode)
			cwrap(cbase ^ 0x9800, V);
			else cwrap(cbase ^ 0x1C00, V);
...
{
if(A<0x2000)
 setchr1(A,(EXPREGS[0]<<8)+V);
else
 setchr1(A,(EXPREGS[1]<<8)+V);
}
else
