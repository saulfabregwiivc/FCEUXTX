...
void FCEU_VSUniToggleDIP(int w) {
	if (GameInfo->type != GIT_VSUNI) {
		FCEU_DispMessage("", 0); //FCEU_DispMessage("Not Vs. System; toggle DIP switch.", 0);
		return;
	}
	vsdip ^= 1 << w;
	DIPS_howlong = 180;
...
void FCEU_VSUniCoin(uint8 slot) {
	if (GameInfo->type != GIT_VSUNI) 
		FCEU_DispMessage("", 0); //FCEU_DispMessage("Not Vs. System; can't insert coin.", 0);
	else {
		switch (slot) {
		case 0:
			coinon = 6; break;
...
void FCEU_VSUniService() {
	if (GameInfo->type != GIT_VSUNI)
		FCEU_DispMessage("", 0); //FCEU_DispMessage("Not Vs. System; can't press service button.", 0);
	else
		service = 6;
}
